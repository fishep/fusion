...
class OrderTest {
    ...
    void newOrder() {
        ...
        orderProducts.add(new OrderProduct(new ProductId(), new Quantity(Quantity.Unit.PIECES, 1), new Money("CNY", 2)));
        orderProducts.add(new OrderProduct(new ProductId(), new Quantity(Quantity.Unit.PIECES, 2), new Money("CNY", 3)));
        ...
        order.addProduct(new OrderProduct(new ProductId(), new Quantity(Quantity.Unit.PIECES, 3), new Money("CNY", 4)));
        assertEquals(20, order.getAmount().getValue());

        assertThrows(Exception.class, () -> {
            order.addProduct(new OrderProduct(new ProductId(), new Quantity(Quantity.Unit.PIECES, 1), new Money("USD", 1)));
        });
